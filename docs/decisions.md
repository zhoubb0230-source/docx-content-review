...
## ADR-049　语料不能外传时，反馈的是结构指纹与计数

- **日期**：2026-08-19
- **起因**：用户问「在材料内容不能提供的前提下，我可以获取哪些有用信息给你」。
  这正是 ADR-048 划出的那半边——**扫查够不到的缺陷全部需要真实语料**，
  而语料通常不能外传。这条阻塞不解开，剩下的优化就只能靠猜。
- **决策**：新增 `scripts/diagnose.py`，产出一份**不含正文**的诊断包。
- **不含正文必须是可验证的，不能是承诺**：`_assert_no_text()` 把包里每个字符串
  拿去撞 `paragraphs.jsonl`，撞上即以退出码 8 终止。回归第 30 节配了负向对照
  （塞一句真正文进去，自检必须抓到）——**加校验必须同时加负向对照**（CLAUDE.md）。
- **收哪几项，是按"哪些缺陷需要它"倒推的**：

  | 项 | 对应的缺陷 |
  |---|---|
  | 图表编号方案（章-序 / 流水各多少处） | ADR-047②：流水式编号让已知集为空，每条引用都"不存在" |
  | 疑似平行表数（表头形状相同的表） | ADR-047①：平行表的同名指标被判成前后矛盾 |
  | 漏斗（raw → 各闸门 → 复核 → 最终） | "模型没查出来"与"查出来被闸门丢光"在报告里长得一样，处置相反 |
  | 台账各类计数 + 填了 scope 的比例 + 幻觉闸门丢弃数 | ADR-047③、ADR-048：某类为 0 往往是没抽出来，而覆盖性规则建立在它之上 |
  | 错别字候选按**词表条目**聚合 | 误报极度集中在少数条目上；条目是我们自己的词表资产，不是文档内容 |

- **刻意不收的**：原文、标题文字、术语、批注内容、`issues.xlsx`、完整报告。
  样式只报**类别**（toc/heading/caption/code/quote/other），不报名字——
  自定义样式名常带单位或项目代号。
- **实跑验证**：在 211,586 字符的语料上跑通，`未识别到任何图表编号` 与
  `疑似平行表 161 张` 两项**当场就指向了用户上报的那两类误报**。
  这两个数字如果早一轮拿到，ADR-047 的①②不需要用户描述现场就能定位。
- **SKILL.md 同时写明还该向用户要什么**（按性价比排序：误报的规则号 + 一句话理由 >
  行业术语表 > 编号与排版约定 > 漏报的类别与量级），并明确**不要主动索取原文**。
- **影响**：新增 `scripts/diagnose.py`、`SKILL.md`（新增一节 + 契约表）、
  `tests/run_regression.sh` 第 30 节。

## ADR-050　落笔跨度不是定位跨度；页码不是估算就别写成确定的

- **日期**：2026-08-19
- **来源**：用户实跑反馈两条——错别字批注圈住一整段；数值冲突批注里引的那句话
  在所指页码上找不到。

### ① 一个两字的错字，落成了删掉整句再插入整句的修订

`typo_scan._unique_window` 把候选窗口撑到「段内唯一」（最宽 ±40 字），
而 `merge` 直接拿这个窗口当 `original_text`/`suggested_text`。于是：

- 修订是「删掉八十多字、再插入八十多字」，Word 里整句被划掉重写；
- 批注锚在这处修订上，高亮跟着覆盖同样的范围。

**窗口的职责是消歧，不是编辑范围。** 这与 ADR-031 的根因是同一句话的另一半：
那次是「把定位用的 run 集合当成锚定用的 run 集合」，这次是把定位用的**文本跨度**
当成了编辑跨度。

- **决策**：`locate_span` / `isolate_span` 增加 `occurrence`（段内第几处）。
  候选带上序号，`merge` 的落笔跨度就是错字本身，`apply_revisions` 按序号定位——
  同段出现两次也各自落笔，不再整条拒绝（原来只能靠撑宽窗口，撑不出唯一就放弃）。
- **A1 的跨度下限必须独立**（`typo_check.min_span_chars`，默认 2）。
  `verification.min_span_chars: 4` 挡的是"模型圈了一两个字、没有依据"，
  而 A1 的跨度来自确定性词表、模型只答 A/B。共用一个下限等于逼着这条通道撑宽跨度。
  **这与 ADR-034 的 `max_span_chars` 误杀 P 类是同一个形状**：
  为 A/B 类定的阈值被无差别套到别的通道上。
- **批注范围取「这处改动所在的整句」**：只圈 del/ins 高亮只有两三个字，
  看不出改的是哪句；圈整段又回到用户反馈的那条。修订落笔后句子被劈成两截，
  范围从前截拉到后截，中间的 del/ins 自然被圈进来。扩不成就退回修订节点，**不猜**。
- 顺带修掉一个真缺陷：`top_level_node(...) or first` —— **lxml 元素不能用真值判断**，
  没有子节点的元素是「假」的，会静默回退到修订节点，表现为"有时圈整句、有时只圈两个字"
  且不报错。

### ② 页码：文档信息越全，逐段页码反而越不准

`page_density()` 按 app.xml → 分页符 → 常数 400 三级降级，**而逐段页码只在
降级到「分页符」那一档时才按分页标记算**；app.xml 给得出总页数时走的是
「字符偏移 ÷ 每页字符数」的线性路径。

于是一份 Word 存过盘的长文档（app.xml 与 `lastRenderedPageBreak` 都在）走的是
最差的那条：线性模型假设字符均匀分布，而带表格、图片、分节符的文档远非如此，
正文中段能偏出几十页。现场反馈就是这个。

- **决策**：把两个问题拆开。**总页数**优先 app.xml；**逐段页码**优先分页标记
  （`lastRenderedPageBreak` 覆盖全部页边界，逐段精确），一个标记都没有才退回线性推算。
- `count_page_breaks` 改为分别返回「渲染分页」与「手动分页」，**两者不能相加**——
  手动分页处 Word 同样会写一个 `lastRenderedPageBreak`，加起来等于把那一页数了两遍。
- **推算出来的页码不能写成确定的页码**：新增 `_common.page_ref()`，
  精确时写「第 36 页」，推算时写「约第 36 页」，缺字段时按推算处理（默认保守）。
  评审人照着一个精确写法的页码翻过去找不到东西，只会认为这条是误报——
  这条误报是措辞造出来的，不是判定造出来的。
- `page_estimated` 一路带到闸门产物与 `ctx.side()`，否则下游只能假定它是精确的。

### 覆盖

回归第 31 节：落笔跨度是错字本身、同段两处各自落笔、两字跨度过得了闸门②、
批注圈住的是被改的那一句（不是整段也不只是那两个字）、四项校验通过；
页码措辞三种情形；有分页标记时逐段精确 + **负向对照**（去掉标记必须退回推算并标为估算）。
