...
> 回归 82/82。§五（优先级序列）已采纳。落地细节见 ADR-021…025 与 `progress.md`。
>
> **§四（B6）的判断被首轮基线证实了**：正样本集上三条 A 类漏报全部落在
> "确实有错、但没有唯一最小修改"的句子上，且答案清单为它们给出的建议
> 被闸门②按 A5 距离上限降级——闸门是对的，缺的是一个只出批注的出口。
> 仍按原计划推迟，但依据已从推测变成数据。
