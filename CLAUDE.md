...
### 两类缺陷已经变成可枚举的扫查，加东西时要顺手补一行

ADR-048 把复发的两个形状固化成了回归：

| 形状 | 守卫 | 加什么东西时必须补 |
|---|---|---|
| 同一条判据只落实了一部分兄弟 | 第 27 节：四个阶段一起断言 `done_marker == output` | 加一个阶段 / 通道 |
| 证据基为空时照样下结论 | 第 29 节：逐条清空证据基，断言闭嘴 + 非空的负向对照 | **加一条「声称某物不存在」的规则** |

第二条特别容易漏，因为它不报错、只报一堆：「我在集合 S 里没找到 X」——
S 为空时说明的是"我一个都没认出来"，不是"文档里没有 X"，而此时每一条 X 都成立。
L15 在流水式图号的文档上就是这么满屏误报的。
