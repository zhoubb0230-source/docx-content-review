...
# --------------------------------------------------------------------------
def split(blocks: list[dict], cfg: dict) -> list[list[dict]]:
    ch = cfg.get("chunking") or {}
    ratio = float(ch.get("token_fallback_ratio") or 1.6)
    max_text = int(ch.get("max_text_tokens") or 15000)
    max_ctx = int(ch.get("max_context_tokens") or 45000)
    overhead = (int(ch.get("instruction_tokens") or 9000)
                + int(ch.get("glossary_tokens") or 4000)
                + int(ch.get("output_reserve_tokens") or 12000)
                + 1000)  # 上文衔接预算
    budget = min(max_text, max(1000, max_ctx - overhead))
    ...
    for b in blocks:
        ...
        if bt > budget:
            ...
            chunks.append([b])
            continue
        # 主切点 H2、次切点 H3：已过半即在标题处断开，让分片对齐章节
        is_split_point = b["kind"] == "heading" and (b["paras"][0]["level"] or 9) <= 3
        if cur and (cur_tok + bt > budget or (is_split_point and cur_tok > budget * 0.5)):
            chunks.append(cur)
            cur, cur_tok = [], 0
        cur.append(b)
        cur_tok += bt
...
def build(run_dir: Path, cfg: dict) -> dict:
    ...
    groups = [blocks] if single_pass else split(blocks, cfg)
...
def main(argv: list[str]) -> int:
    ...
    emit({"ok": True, **build(Path(args.run_dir).resolve(), load_config(args.config))})
    return EX.OK
