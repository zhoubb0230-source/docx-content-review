...
`locate_span` 取的是**首个**匹配。跨度在同一段里出现多次时，"改哪一处"没有依据——
问题记录里只有 `pid` 与 `original_text`，没有偏移量。

> 「本期指标目标为 **200ms**，实测值为 1**200ms**。」
> 把「200ms」改掉，改中的是目标值还是实测值，取决于 `find` 而不是取决于判定。

因此 `ooxml.span_count()` 先数一遍，`apply_revisions.py` 据此分三种处置：

| 出现次数 | 处置 |
|---|---|
| 1 | 照常落笔 |
| >1，且补丁带 `occurrence` | 落在**指定的那一处**（错别字与扩散通道逐处出条目，各带各的序号） |
| >1，且补丁带 `all_occurrences` | **逐处全换**（术语规范化 L25/L26：语义本来就是"这个写法全换掉"） |
| >1，A1 且两者都没有 | 按第 0 处落笔——A1 的判定对象是**字串本身**，同段同形的每一处都是同一个错，"改哪一处"不成疑问；其余各处由 `typo_scan.py propagate` 补成独立条目 |
| >1，其余情形 | **不落笔**，记为失败并附出现次数；照常出普通批注，不静默消失 |

**同段多处必须从后往前落笔。** 序号按原始段落文本算，而每落一笔，被替换的那一处
就进了 `w:del`、不再参与定位——先改第 0 处，第 1 处就变成「该段共 1 处，
指定的第 2 处不存在」，后面各处全数落空。`apply_revisions.py apply` 因此按
`occurrence` 降序排一遍再落笔（稳定排序，其余顺序不变）。

整段替换的循环不需要额外记账：换完一处后原文已进 `w:del`，而 `para_runs` 跳过
修订标记内的 run，下一轮 `locate_span` 自然找到下一处。

批注锚点同理，但**判定必须放在 `plan` 里、依据原始段落文本**：`apply` 的时候
段落已经被修订改过，在那时判会得出与计划不一致的结论（计划说"说不清是哪一处"，
落笔却精确圈住了剩下的那一处）。`commentlist.json` 由 `plan` 写、
`validate_docx` 也拿它做预期，三者必须一致。
