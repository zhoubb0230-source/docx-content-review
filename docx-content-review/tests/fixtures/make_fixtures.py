...
def build_basic(outdir: Path) -> None:
    ...
    # A1 错别字
    pos("平台已完成部署，用户可通过帐号登录后使用全部功能。", "A1", "帐号", "账号", "帐/账 形近误用")
    pos("安装程序会自动按装依赖组件，无需人工干预。", "A1", "按装", "安装", "")
    pos("系统布署完成后需重启服务进程。", "A1", "布署", "部署", "")
    # A2 的地得
    ...
    # A4 成分残缺
    pos("通过本次架构升级，使系统的整体吞吐能力得到显著提升。", "A4", "通过本次架构升级，使系统",
        "本次架构升级使系统", "介词导致主语残缺")
    pos("为了保证数据安全，必须对所有出域数据进行。", "A4", "进行。", "进行加密。", "缺宾语中心语")
    pos("能否顺利上线，取决于团队的持续投入。", "A4", "能否顺利上线，取决于团队的持续投入",
        "能否顺利上线，取决于团队能否持续投入", "两面对一面")
    # A5 搭配不当
    ...
    pos("该模块目前正在开发中，预计下月完成。", "A7", "目前正在开发中", "正在开发", "目前/正在/中叠用")
    pos("这是一个非常十分关键的技术选型决策。", "A7", "非常十分关键", "非常关键", "")
    # A8 数字单位
    pos("核心链路端到端时延不超过 200ms，边缘链路不超过 0.5s。", "A8", "0.5s", "500ms", "同一 subject 单位不统一")
    pos("本期预算 1,00,000 元，已完成审批。", "A8", "1,00,000", "100,000", "千分位错位")
    # B 类
