...
def build_logic(outdir: Path) -> None:
    ...
    # L29/L30 的证据侧。少了这两句，「有对应关系」与「有验收指标」在全篇是空集，
    # 而空集之下每个目标都"没人认领"、每条举措都"缺验收"——那时这两条断言
    # 通过的是缺陷，不是规则（见 ADR-048）。
    para("为实现目标 O2（缺陷率下降 15%），本期开展举措 I8：建设统一构建平台。")
    para("验收标准：I8 的构建成功率不低于 99%。")
